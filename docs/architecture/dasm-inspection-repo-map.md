...
- **التزامات تقنية حقيقية (ما علينا):** مزامنة Core تفشل بصمت بلا توكن · عدم تطابق نوع مع ريبو السوق (BIGINT ↔ UUID) · متغيّرات بيئة كثيرة بلا توثيق مركزي · حدّ معدّل جزئي · وثائق متناقضة.
...
### ⚠️ ما علينا (التزامات — مرتّبة)
| # | البند | الخطورة | الإصلاح المقترح |
|---|---|---|---|
| 1 | مزامنة Core (تقرير/سيارة/محفظة) **تفشل بصمت** بلا `DASM_INSPECTION_INTERNAL_PULL_TOKEN` | 🔴 عالٍ | تأكيد ضبط التوكن على Vercel + تنبيه/سجلّ عند غيابه بدل الابتلاع الصامت |
| 2 | فجوات الوضع الداكن على لوحة الورشة + تفاصيل الطلب (ألوان فاتحة ثابتة) | 🟡 متوسط | استبدال الألوان الثابتة بأصناف `dark:` (الخطوة التالية الجاهزة) |
| 3 | عدم تطابق نوع مع ريبو السوق (`external_request_id` BIGINT ↔ `inspection_requests.id` UUID) | 🟡 متوسط | حسم الربط عبر هجرة موافَق عليها قبل تفعيل عرض التقارير على الإعلانات |
| 4 | وكلاء المصادقة (login/apple/social-exchange) **بلا حدّ معدّل** + التقاطات صامتة | 🟡 متوسط | إضافة حدّ معدّل + تسجيل الأخطاء |
| 5 | وثائق قديمة/مفقودة: `DATABASE.md` يصف حالة Laravel قديمة · `V1_SCOPE.md` و`IMPLEMENTATION_STATUS.md` مفقودان · `DOMAIN_MODEL.md` مكرّر | 🟡 متوسط | أرشفة/تحديث + إضافة هذه الوثيقة كمصدر حقيقة |
| 6 | اكتشاف `/track/[id]` يعتمد على معرفة الرابط (يتيم مقصود) | 🔵 منخفض | مقبول؛ يُذكر في وثيقة الـSSO |
| 7 | روابط افتراضية ثابتة (api.dasm.com.sa) قد تصيب بيئة خاطئة لو نُسي متغيّر | 🔵 منخفض | التحقّق من متغيّرات البيئة عند الإقلاع |

---

## 10. خطة إعادة الربط (المقترحة، بالترتيب)
1. **تدقيق بيئة Vercel:** التأكّد من ضبط كل متغيّرات القسم 8 — خصوصاً `DASM_INSPECTION_INTERNAL_PULL_TOKEN` و`DASM_GATEWAY_API_KEYS` و`SUPABASE_SERVICE_ROLE_KEY`.
2. **تلميع الوضع الداكن** على لوحة الورشة وتفاصيل الطلب (بند 2) — جاهز للتنفيذ فوراً.
3. **تنبيهات بدل الفشل الصامت** لمزامنة Core (بند 1).
4. **تنظيف الوثائق** (بند 5): أرشفة `DATABASE.md` القديمة، اعتماد هذه الوثيقة، إنشاء `IMPLEMENTATION_STATUS.md`.
5. **حسم ربط السوق** (بند 3) بهجرة موافَق عليها.
6. **تصليب المصادقة** (بند 4): حدّ معدّل + تسجيل.

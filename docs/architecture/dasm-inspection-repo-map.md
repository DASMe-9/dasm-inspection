...
- **التزامات (تحديث 2026-07-07): ✅ كلها حُسمت (1-6 + القشرة):** مزامنة Core (#72) · الوضع الداكن (#73) · تطابق نوع السوق (#74) · حدّ معدّل المصادقة (#76) · تنظيف الوثائق (#78) · فحص البيئة عند الإقلاع (#77) · القشرة التكيّفية (#70). المتبقّي الوحيد إجراء تشغيلي: تأكيد متغيّرات البيئة على Vercel.
...
|---|---|---|---|
| 1 | مزامنة Core (تقرير/سيارة/محفظة) كانت **تفشل بصمت** | 🔴 عالٍ | **✅ حُسم (#72):** حالة مزامنة دائمة على `inspection_reports` (status/synced_at/error/attempts) + التقاط النتيجة + `retryReportCoreSync()` + كسر صمت المحفظة. **التوكن `DASM_INSPECTION_INTERNAL_PULL_TOKEN` ضُبط على Vercel (2026-07-07) ومُتحقَّق حيّاً (`env_check_ok` في لوق الإنتاج).** |
...
| 4 | وكلاء المصادقة (login/apple/social-exchange) **بلا حدّ معدّل** + التقاطات صامتة | 🟡 متوسط | **✅ حُسم (#76):** `proxyAuthToCore` مشترك — حدّ معدّل لكل عنوان (`AUTH_PROXY_RATE_LIMIT_PER_MIN`) + تسجيل أخطاء المنبع/الشبكة (429/502). |
| 5 | وثائق قديمة | 🟡 متوسط | **✅ حُسم (#78):** `DATABASE.md` عليه إشعار إهمال يشير لهذه الوثيقة + `domain-model.md`. تصحيح: `V1_SCOPE.md`/`IMPLEMENTATION_STATUS.md` **موجودان** (تدقيق سابق بالغ)؛ `DOMAIN_MODEL.md` فهرس مقصود لا تكرار. |
| 6 | روابط/توكنات بيئة قد تُنسى على Vercel (يشمل التوكن الداخلي/بند 1) | 🔵 منخفض | **✅ حُسم (#77) + مُتحقَّق:** فحص إقلاع يسجّل المفقود؛ دُقّقت بيئة الإنتاج فعلياً عبر Vercel API، ضُبط التوكن الناقص، والإنتاج الآن `env_check_ok`. |
| 7 | اكتشاف `/track/[id]` يعتمد على معرفة الرابط (يتيم مقصود) | 🔵 منخفض | مقبول؛ يُذكر في وثيقة الـSSO |

---

## 10. خطة إعادة الربط — ✅ مكتملة (2026-07-07)
- ✅ **قشرة تكيّفية** (كسر «الصفحات المستقلة» + الفوتر) — #70.
- ✅ **تأمين مزامنة Core** (بند 1) — #72.
- ✅ **تلميع الوضع الداكن** (بند 2) — #73.
- ✅ **حسم ربط السوق** (بند 3) — #74.
- ✅ **تصليب المصادقة** (بند 4) — #76.
- ✅ **تنظيف الوثائق** (بند 5) — #78.
- ✅ **فحص البيئة عند الإقلاع** (بند 6) — #77.

**✅ بيئة الإنتاج مُدقَّقة (2026-07-07):** ضُبط `DASM_INSPECTION_INTERNAL_PULL_TOKEN` الناقص على Vercel، والإنتاج الآن `env_check_ok`.

## 11. تكافؤ الويب ↔ تطبيق الجوال (Flutter)
- **✅ منفّذ:** توصيات الإصلاح + المحادثة ثنائية الاتجاه على شاشة تفاصيل الطلب (الجوال PR#15 + endpoint ويب `POST /api/mobile/requests/[id]/messages` #80). قائمة الطلبات، الفحص، الصور، تاريخ المركبة/الصيانة/OBD، التذكيرات، الإشعارات، دليل الورش، الاعتماد/الرفض — **متكافئة**.
- **📋 متبقٍّ (أكبر/بعضه ويب-أولاً بالتصميم):** المحفظة · الاشتراك · لوحة الورشة وإدارتها · صفحة تتبّع العميل · الإشراف. كلٌّ يحتاج endpoint جوال + شاشة. لوحة home في الجوال تربط «اللوحة المتقدّمة (ويب)» لهذه عمداً.

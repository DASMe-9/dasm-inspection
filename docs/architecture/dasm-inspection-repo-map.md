...
| 6 | روابط/توكنات بيئة قد تُنسى على Vercel (يشمل التوكن الداخلي/بند 1) | 🔵 منخفض | **✅ حُسم (#77):** فحص إقلاع (`instrumentation.ts` + `env-check.ts`) يسجّل المتغيّرات الحرجة المفقودة. يبقى إجراء تشغيلي: التأكّد من ضبطها فعلاً على Vercel. |
...
**إجراء تشغيلي متبقٍّ (خارج الكود):** التأكّد من ضبط المتغيّرات الحرجة فعلاً على Vercel (سيسجّلها فحص الإقلاع إن غابت).

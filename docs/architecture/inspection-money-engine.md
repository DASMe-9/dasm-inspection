...
## 🔒 إضافات المالك المُلزِمة (2026-07-07، الدفعة الثانية)
1. **عزل السياق:** 2أ/2ب فقط يجوزان في جلسة عامة. **2ج (قلب التدفّق المالي) يبدأ في جلسة مركّزة نظيفة** تحمل القرارات المقفلة من STATE فقط — السياق الطويل + كود المال لا يمتزجان.
2. **هجرة 2ب:** **يُعاد التحقّق من صفر تكرار `(platform, reference)` على `escrow_transactions` لحظة الهجرة مباشرةً قبل تطبيق `UNIQUE`** — لا اعتماد على فحص المرحلة 1.
3. **نطاق المرحلة 3 — استرداد المحفظة منطق جديد:** `HandleRefundSucceededAction` القائم يغطّي **توفيق استرداد PayMob فقط**. مسار **الاسترداد إلى محفظة العميل (الافتراضي الجديد) منطق ليدجر داخليّ غير موجود بعد** — يُدرَج صراحةً في نطاق المرحلة 3 (لا يُفترَض مغطّى).
4. **بوابة العملية (إلزامية):** كل PR **كود** من 2أ فصاعداً: **أدلّة تُعرَض → موافقة المالك → دمج. بلا استثناء (حتى الـ«آمنة»).** PRs التوثيق فقط مُعفاة.
5. **اختبارات 2د:** تغطّي حالة **مالك الورشة موجود مسبقاً كمستخدم عادي** — الربط يجب ألا يُنشئ مستخدماً مكرّراً ولا محفظة ثانية.

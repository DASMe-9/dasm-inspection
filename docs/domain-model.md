...
### `inspection_app_role` (الحالة الحالية في DB)

`super_admin` | `inspection_admin` | `workshop_manager` | `inspector` | `viewer`

**محاذاة مع أدوار DASM المعتمدة:** انظر `permissions-matrix.md` (Phase 2: توحيد التسمية أو توسيع الـ enum دون كسر البيانات).
...
تنفيذ RLS الفعلي في **Phase 2–3** بعد تثبيت شكل التوكن من DASM.

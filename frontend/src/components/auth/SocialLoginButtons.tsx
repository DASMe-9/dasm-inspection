...
/** يُخفى الزرّ تلقائياً إن لم يُضبط متغيّر مزوّده على Vercel. */
export const socialLoginAvailable = googleEnabled || appleEnabled;

const BTN =
  "inline-flex h-12 w-full items-center justify-center gap-2.5 rounded-xl border text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-60";

export function SocialLoginButtons({ onGoogle, onApple, busy, disabled }: Props) {
  if (!socialLoginAvailable) return null;
  const off = disabled || busy !== null;

  return (
    <div className="space-y-4">
      <div
        className={`grid gap-3 ${googleEnabled && appleEnabled ? "sm:grid-cols-2" : "grid-cols-1"}`}
      >
        {googleEnabled && (
          <button
            type="button"

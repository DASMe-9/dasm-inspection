...
import { mobileApproveReport } from "@/lib/api/mobile-inspection-mutations";

/**
 * POST /api/mobile/requests/:id/approve
 */
export async function POST(
  ...
  const result = await mobileApproveReport(id, auth.normalized);
  if (!result.ok) {
    return NextResponse.json(
